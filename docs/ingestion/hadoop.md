...
|maxBytesInMemory|Long|The number of bytes to aggregate in heap memory before persisting. If this value is between 0 and 100 then it is treated as percentage of max JVM memory. Normally this is computed internally and user does not need to set it. This is based on a rough estimate of memory usage and not actual usage. The maximum heap memory usage for indexing is maxBytesInMemory * (2 + maxPendingPersists).|no (default == One-sixth of max JVM memory)|
