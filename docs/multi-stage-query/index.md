...
#### Durable storage for shuffle mesh

If the cluster is running queries which have > 20 workers always, consider enabling the experimental durable storage
shuffle mesh. The shuffle mesh is activated when `talariaDurableShuffleStorage=true` is present in the query context.
Currently, only s3 is supported as the durable storage.
The properties below should be put under common configurations in imply manager.

##### S3

The `druid-s3-extensions` must be loaded to use s3 for shuffle mesh.

|config| description                                                                                                                                                                                                                                                                                       |required| default     |
| --- |---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| --- |-------------|
|`druid.talaria.intermediate.storage.type`| Must be set to `s3`.                                                                                                                                                                                                                                                                              | yes | none |
|`druid.talaria.intermediate.storage.bucket`| S3 bucket to store intermediate stage results.                                                                                                                                                                                                                                                    | yes |             |
|`druid.talaria.intermediate.storage.prefix`| S3 prefix to store intermediate stage results. Do not share the same path with other clusters.                                                                                                                                                                                                    | yes |             |
|`druid.talaria.intermediate.storage.tempDir`| Directory path in local disk to store intermediate stage results temporarily.                                                                                                                                                                                                                     | yes |             |
|`druid.talaria.intermediate.storage.maxResultsSize`| Max size of each result file. It should be between 5MiB and 5TiB. Supports human-readable format.                                                                                                                                                                                                 | no | 100MiB      |
|`druid.talaria.intermediate.storage.chunkSize`| This property is intended only to support rare cases. This property defines the size of each chunk to temporarily store in `druid.talaria.intermediate.storage.tempDir`. Druid computes the chunk size automatically when this property is not set. The chunk size must be between 5MiB and 5GiB. | no | null        |
|`druid.talaria.intermediate.storage.maxTriesOnTransientErrors`| This property is intended only to support rare cases. This property defines the max number times to attempt s3 API calls to avoid failures due to transient errors.                                                                                                                               | no | 10          |
...
druid.talaria.intermediate.storage.maxResultsSize=1GiB
```

##### S3 permissions/lifecycle settings

`s3:GetObject`, `s3:PutObject`, and `s3:AbortMultipartUpload` are required for pushing/fetching intermediate stage
results to/from S3.
`s3:DeleteObject` is required for removing the intermediate stage result after usage.
...
| talariaMode                  | (SELECT or INSERT)<br /><br />Execute a query with a predefined set of parameters which are pretuned. It can be set to `strict` or `non-strict`. Please check the [Talaria Modes](#modes) section for more information.<br /><br />                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | strict                                                               |
...
| talariaDurableShuffleStorage | (SELECT OR INSERT OR REPLACE) <br /><br />Whether to use [durable storage for shuffle mesh](#durable-storage-for-shuffle-mesh). To use this parameter, durable storage must be configured at the server level using `druid.talaria.intermediate.storage.enable=true` and other associated properties. If these properties are not configured, the query with the context property `talariaDurableShuffleStorage=true` will fail with a configuration error. <br /><br />                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | false                                                                |
## API
...
|  DurableStorageConfiguration  | Durable storage mode could not be activated due to a misconfiguration.<br /><br /> Check [durable storage for shuffle mesh](#durable-storage-for-shuffle-mesh) for instructions on configuration.                                                                                                                                                                                                                                                                   |  |
...
|  TaskStartTimeout  | Unable to launch all the worker tasks in time. <br /> <br />There might be insufficient available slots to start all the worker tasks simultaneously.<br /> <br /> Try splitting up the query into smaller chunks with lesser `talariaNumTasks` number. Another option is to increase capacity.                                                                                                                                                                     | |
...
- In case of a worker crash, stage outputs on s3 would not be deleted automatically. In this case users may need to delete
  the file using an external process or create a s3 lifecyclepolicy to remove the objects
