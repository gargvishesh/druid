...
|`druid.auth.keycloak.common.initialRoleMappingFile`| This setting specifies the path to a JSON file which may be used to import role to permission mappings when the extension starts up. If a role specified in the file already exists, then its permissions are overwritten. Note that `druid.auth.keycloak.common.autoPopulateAdmin` must also be set to true for this file to be imported. | n/a |
