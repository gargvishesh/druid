...
public class ImplyAutoScalerTest
{
  ...
  @Mock
  private ImplyEnvironmentConfig mockImplyEnvironmentConfig;

  private ImplyAutoScaler implyAutoScaler;

  @Before
  public void setUp()
  {
    implyAutoScaler = new ImplyAutoScaler(
        MIN_NUM_WORKER,
        MAX_NUM_WORKER,
        mockImplyEnvironmentConfig,
  ...
  public void testSerde() throws Exception
  {
    ...
    mapper.registerSubtypes(ImplyAutoScaler.class);
  ...
  {
    EqualsVerifier.forClass(ImplyEnvironmentConfig.class).withNonnullFields(
  ...
  public void testIpToId() throws Exception
  {
    ...
    List<String> ids1 = implyAutoScaler.ipToIdLookup(ips1);
    ...
    Mockito.when(mockImplyManagerServiceClient.listInstances(ArgumentMatchers.eq(mockImplyEnvironmentConfig))).thenReturn(mockResponse);

    List<String> ids3 = implyAutoScaler.ipToIdLookup(ImmutableList.of(IP_1));
    Assert.assertEquals(1, ids3.size());
    Assert.assertEquals(ID_1, ids3.get(0));

    Mockito.verify(mockImplyManagerServiceClient).listInstances(ArgumentMatchers.eq(mockImplyEnvironmentConfig));
  ...
  public void testIdToIp() throws Exception
  {
    ...
    List<String> ips1 = implyAutoScaler.idToIpLookup(ids1);
    ...
    List<String> ids3 = implyAutoScaler.idToIpLookup(ImmutableList.of(ID_1));
    Assert.assertEquals(1, ids3.size());
  ...
  public void testTerminateWithIdsEmpty()
  {
    // empty Ids
    List<String> ids1 = Collections.emptyList();
    implyAutoScaler.terminateWithIds(ids1);
    Mockito.verifyNoMoreInteractions(mockImplyManagerServiceClient);
  }
  ...
  {
    // non-empty ids
    implyAutoScaler.terminateWithIds(ImmutableList.of(ID_1, ID_2));

    // verify
    ArgumentCaptor<String> idsCaptor = ArgumentCaptor.forClass(String.class);
    Mockito.verify(mockImplyManagerServiceClient, Mockito.times(2)).terminateInstances(ArgumentMatchers.eq(mockImplyEnvironmentConfig), idsCaptor.capture());
    Mockito.verifyNoMoreInteractions(mockImplyManagerServiceClient);
  ...
  public void testTerminateWithEmpty()
  {
    // empty Ips
    List<String> ips1 = Collections.emptyList();
    implyAutoScaler.terminate(ips1);
    Mockito.verifyNoMoreInteractions(mockImplyManagerServiceClient);
  }
  ...
  public void testTerminateWithNonEmpty() throws Exception
  {
    ...
    // non-empty ips
    implyAutoScaler.terminate(ImmutableList.of(IP_1, IP_2));
  ...
  public void testProvisionExceedMaximum()  throws Exception
  {
    ...
    AutoScalingData actual = implyAutoScaler.provision();
  ...
  public void testProvisionUnderMaximum()  throws Exception
  {
    ...
    Mockito.when(mockImplyManagerServiceClient.listInstances(ArgumentMatchers.eq(mockImplyEnvironmentConfig))).thenReturn(mockResponse);
    Mockito.when(mockImplyManagerServiceClient.provisionInstances(ArgumentMatchers.any(ImplyEnvironmentConfig.class), ArgumentMatchers.anyString(), ArgumentMatchers.anyInt())).thenReturn(ImmutableList.of(ID_3));
    ...
    Mockito.verify(mockImplyManagerServiceClient).listInstances(ArgumentMatchers.eq(mockImplyEnvironmentConfig));
    Mockito.verify(mockImplyManagerServiceClient).provisionInstances(ArgumentMatchers.eq(mockImplyEnvironmentConfig), ArgumentMatchers.eq(WORKER_VERSION), ArgumentMatchers.eq(1));
    Mockito.verifyNoMoreInteractions(mockImplyManagerServiceClient);
  }
  ...
  public void testGetStartingOrRunningInstances()  throws Exception
  {
    ...
    List<String> actual = implyAutoScaler.getStartingOrRunningInstances();
    Assert.assertNotNull(actual);
