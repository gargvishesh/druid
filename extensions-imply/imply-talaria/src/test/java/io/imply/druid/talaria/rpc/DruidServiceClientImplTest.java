...
public class DruidServiceClientImplTest
{
  ...
  private DruidServiceClient serviceClient;
  ...
  public void test_request_ok()
  {
    ...
    final Either<RpcServerError, Map<String, String>> response = doRequest(serviceClient, requestBuilder);

    Assert.assertTrue(response.isValue());
    Assert.assertEquals(expectedResponseObject, response.valueOrThrow());
  }
  ...
  public void test_request_serverError()
  {
    ...
    serviceClient = makeServiceClient(new StandardRetryPolicy(2));
    final Either<RpcServerError, Map<String, String>> response = doRequest(serviceClient, requestBuilder);

    Assert.assertTrue(response.isError());
    Assert.assertEquals(HttpResponseStatus.INTERNAL_SERVER_ERROR, response.error().getResponse().getStatus());
    Assert.assertEquals("oh no", response.error().getResponse().getContent());
  }

  @Test
  public void test_request_serverErrorRetry()
  {
    final RequestBuilder requestBuilder = new RequestBuilder(HttpMethod.GET, "/foo");
    final ImmutableMap<String, String> expectedResponseObject = ImmutableMap.of("foo", "bar");
  ...
  public void test_request_ioError()
  {
    ...
    MatcherAssert.assertThat(e, ThrowableCauseMatcher.hasCause(CoreMatchers.instanceOf(RpcException.class)));
    MatcherAssert.assertThat(e.getCause(), ThrowableCauseMatcher.hasCause(CoreMatchers.instanceOf(IOException.class)));
    MatcherAssert.assertThat(
        e.getCause().getCause(),
  ...
  @Test
  public void test_request_ioErrorRetry()
  {
    final RequestBuilder requestBuilder = new RequestBuilder(HttpMethod.GET, "/foo");
    final ImmutableMap<String, String> expectedResponseObject = ImmutableMap.of("foo", "bar");
  ...
  @Test
  public void test_request_nullResponseFromClientRetry()
  {
    final RequestBuilder requestBuilder = new RequestBuilder(HttpMethod.GET, "/foo");
    final ImmutableMap<String, String> expectedResponseObject = ImmutableMap.of("foo", "bar");
  ...
  @Test
  public void test_request_followRedirect()
  {
    final RequestBuilder requestBuilder = new RequestBuilder(HttpMethod.GET, "/foo");
    final ImmutableMap<String, String> expectedResponseObject = ImmutableMap.of("foo", "bar");
  ...
  public void test_request_tooManyRedirects()
  {
    ...
    MatcherAssert.assertThat(e, ThrowableCauseMatcher.hasCause(CoreMatchers.instanceOf(RpcException.class)));
    MatcherAssert.assertThat(e, ThrowableMessageMatcher.hasMessage(CoreMatchers.containsString("too many redirects")));
  }
  ...
  @Test
  public void test_request_serviceUnavailableRetry()
  {
    final RequestBuilder requestBuilder = new RequestBuilder(HttpMethod.GET, "/foo");
    final ImmutableMap<String, String> expectedResponseObject = ImmutableMap.of("foo", "bar");
  ...
  public void test_request_cancelBeforeServiceLocated()
  {
    ...
    final ListenableFuture<Either<RpcServerError, Map<String, String>>> response =
        doAsyncRequest(serviceClient, requestBuilder);
  ...
  {
    Assert.assertEquals(100, DruidServiceClientImpl.computeBackoffMs(0));
    Assert.assertEquals(200, DruidServiceClientImpl.computeBackoffMs(1));
    Assert.assertEquals(3200, DruidServiceClientImpl.computeBackoffMs(5));
    Assert.assertEquals(30000, DruidServiceClientImpl.computeBackoffMs(20));
  }

  @Test
  public void test_serviceLocationNoPathFromUri()
  {
    Assert.assertNull(DruidServiceClientImpl.serviceLocationNoPathFromUri("/"));

    Assert.assertEquals(
        new ServiceLocation("1.2.3.4", 9999, -1, ""),
        DruidServiceClientImpl.serviceLocationNoPathFromUri("http://1.2.3.4:9999/foo")
    );

    Assert.assertEquals(
        new ServiceLocation("1.2.3.4", 80, -1, ""),
        DruidServiceClientImpl.serviceLocationNoPathFromUri("http://1.2.3.4/foo")
    );

    Assert.assertEquals(
        new ServiceLocation("1.2.3.4", -1, 9999, ""),
        DruidServiceClientImpl.serviceLocationNoPathFromUri("https://1.2.3.4:9999/foo")
    );

    Assert.assertEquals(
        new ServiceLocation("1.2.3.4", -1, 443, ""),
        DruidServiceClientImpl.serviceLocationNoPathFromUri("https://1.2.3.4/foo")
    );
  }
  ...
  {
    Assert.assertTrue(DruidServiceClientImpl.isRedirect(HttpResponseStatus.FOUND));
    Assert.assertTrue(DruidServiceClientImpl.isRedirect(HttpResponseStatus.MOVED_PERMANENTLY));
    Assert.assertTrue(DruidServiceClientImpl.isRedirect(HttpResponseStatus.TEMPORARY_REDIRECT));
    Assert.assertFalse(DruidServiceClientImpl.isRedirect(HttpResponseStatus.OK));
  }
  ...
  private DruidServiceClient makeServiceClient(final RetryPolicy retryPolicy)
  {
    return new DruidServiceClientImpl(SERVICE_NAME, httpClient, serviceLocator, retryPolicy, exec);
  }

  private static Either<RpcServerError, Map<String, String>> doRequest(
  ...
  private static ListenableFuture<Either<RpcServerError, Map<String, String>>> doAsyncRequest(
      final DruidServiceClient serviceClient,
      final RequestBuilder requestBuilder
  )
  {
