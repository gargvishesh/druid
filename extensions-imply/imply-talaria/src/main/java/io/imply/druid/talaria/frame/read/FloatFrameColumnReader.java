...
import io.imply.druid.talaria.frame.write.FloatFrameColumnWriter;
...
public class FloatFrameColumnReader implements FrameColumnReader
{
  ...
  public ColumnPlus readColumn(final Frame frame)
  {
    ...
    return new ColumnPlus(
        new FloatFrameColumn(frame, hasNulls, memoryRange),
  ...
  private static class FloatFrameColumn implements NumericColumn
  {
    ...
    )
    {
      ...
      this.memoryPosition = memoryRange.start() + FloatFrameColumnWriter.DATA_OFFSET;
    }
    ...
    private float getFloat(final int physicalRow)
    {
      ...
      if (hasNulls) {
        return memoryRange.memory().getFloat(rowPosition + 1);
      } else {
        return memoryRange.memory().getFloat(rowPosition);
      }
    }
  }
}
