...
import io.imply.druid.talaria.frame.write.LongFrameColumnWriter;
...
public class LongFrameColumnReader implements FrameColumnReader
{
  ...
  public ColumnPlus readColumn(final Frame frame)
  {
    ...
    return new ColumnPlus(
        new LongFrameColumn(frame, hasNulls, memoryRange),
  ...
  private void validate(final Frame frame)
  {
    ...
    if (memorySize < LongFrameColumnWriter.DATA_OFFSET) {
    ...
    if (memorySize != LongFrameColumnWriter.DATA_OFFSET + (long) sz * frame.numRows()) {
  ...
  private static class LongFrameColumn implements NumericColumn
  {
    ...
    )
    {
      ...
      this.memoryPosition = memoryRange.start() + LongFrameColumnWriter.DATA_OFFSET;
    }
    ...
    private long getLong(final int physicalRow)
    {
      ...
      if (hasNulls) {
        return memoryRange.memory().getLong(rowPosition + 1);
      } else {
        return memoryRange.memory().getLong(rowPosition);
      }
    }
  }
}
