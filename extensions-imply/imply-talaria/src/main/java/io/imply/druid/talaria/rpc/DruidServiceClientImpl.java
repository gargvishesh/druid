...
/**
 * "Mid-level" client that wraps an {@link HttpClient} that uses a {@link ServiceLocator} instead of a fixed URL, and
 * that can use a {@link RetryPolicy} to retry failed requests.
 *
 ...
 */
public class DruidServiceClientImpl implements DruidServiceClient
{
  private static final Logger log = new Logger(DruidServiceClient.class);
   ...
   */
  @Override
  public <IntermediateType, FinalType> ListenableFuture<Either<RpcServerError, FinalType>> asyncRequest(
      final RequestBuilder requestBuilder,
      final HttpResponseHandler<IntermediateType, FinalType> handler
  )
  {
    final SettableFuture<Either<RpcServerError, FinalType>> retVal = SettableFuture.create();
    tryRequest(requestBuilder, handler, retVal, 0, 0);
    return retVal;
  }

  @Override
  public DruidServiceClientImpl withRetryPolicy(RetryPolicy newRetryPolicy)
  {
    return new DruidServiceClientImpl(serviceName, httpClient, serviceLocator, newRetryPolicy, connectExec);
  }

  private <IntermediateType, FinalType> void tryRequest(
      final RequestBuilder requestBuilder,
      final HttpResponseHandler<IntermediateType, FinalType> handler,
      final SettableFuture<Either<RpcServerError, FinalType>> retVal,
    ...
    whenServiceReady(
        serviceLocation -> {
          ...
          if (serviceLocation == null) {
            ...
            final long backoffMs = computeBackoffMs(attemptNumber);

            if (isUnlimitedRetry() || nextAttemptNumber < retryPolicy.maxAttempts()) {
              log.info(
              ...
              retVal.setException(new RpcException("Service [%s] is not available", serviceName));
            }
          ...
          Futures.addCallback(
              ...
              new FutureCallback<Either<StringFullResponseHolder, FinalType>>()
              {
                ...
                public void onSuccess(@Nullable final Either<StringFullResponseHolder, FinalType> result)
                {
                  try {
                    ...
                    if (result != null && result.isValue()) {
                      ...
                      retVal.set(Either.value(result.valueOrThrow()));
                    } else {
                      ...
                      if (errorHolder != null && isRedirect(errorHolder.getResponse().getStatus())) {
                      ...
                      } else if ((isUnlimitedRetry() || nextAttemptNumber < retryPolicy.maxAttempts())
                        ...
                        retVal.set(Either.error(new RpcServerError(errorHolder)));
                      } else {
                ...
                {
                  try {
                    final int numAttempts = attemptNumber + 1;

                    if (isUnlimitedRetry()
                        || (numAttempts < retryPolicy.maxAttempts() && retryPolicy.retryThrowable(t))) {
                      final long backoffMs = computeBackoffMs(attemptNumber);

                      log.noStackTrace().info(t, buildErrorMessage(request, null, backoffMs, numAttempts));
                      ...
                      retVal.setException(new RpcException(t, buildErrorMessage(request, null, -1, numAttempts)));
                    }
                  }
                  catch (Throwable t2) {
                    // It's a bug if this happens. The purpose of this line is to help us debug what went wrong.
        ...
        new FutureCallback<ServiceLocations>()
        {
          ...
          public void onSuccess(final ServiceLocations locations)
          {
            if (locations.isClosed()) {
              retVal.setException(new RpcException("Service [%s] is closed", serviceName));
              return;
            }
  ...
  private boolean isUnlimitedRetry()
  {
  ...
  )
  {
    ...
    if (errorHolder != null) {
      errorMessage.append("; ").append(RpcServerError.choppedBodyErrorMessage(errorHolder.getContent()));
    }
  ...
  @VisibleForTesting
  static long computeBackoffMs(final int attemptNumber)
  {
    return Math.max(
        INITIAL_BACKOFF_MS,
        Math.min(MAX_BACKOFF_MS, (long) (Math.pow(2, attemptNumber) * INITIAL_BACKOFF_MS))
    );
  }
