...
public class LongFrameColumnWriter implements FrameColumnWriter
{
  ...
  public int compare(int row1, int row2)
  {
    ...
    if (hasNulls) {
      ...
      return Long.compare(region1.getLong(regionPosition1 + 1), region2.getLong(regionPosition2 + 1));
    } else {
      return Long.compare(appendableMemory.getLong(position1), appendableMemory.getLong(position2));
    }
  }
  ...
  public void writeTo(WritableByteChannel channel) throws IOException
  {
    ...
    buf.put(FrameColumnWriters.TYPE_LONG).put(hasNulls ? (byte) 1 : (byte) 0).flip();
