 ...
 * {@link io.imply.druid.talaria.frame.processor.SuperSorter}, and one assuming it'll be used for a regular
...
public class WorkerMemoryParameters
{
  ...
  /**
   * Fraction of free memory per bundle that can be used by {@link io.imply.druid.talaria.querykit.BroadcastJoinHelper}
   ...
   *
   * Throws a {@link TalariaException} with an appropriate fault if the provided combination of parameters cannot
  ...
  )
  {
    ...
    if (bundleMemoryForProcessing < PROCESSING_MINIMUM_BYTES) {
      ...
      if (maxWorkers > 0) {
        throw new TalariaException(new TooManyWorkersFault(numInputWorkers, Math.min(Limits.MAX_WORKERS, maxWorkers)));
      } else {
    ...
    if (maxNumFramesForSuperSorter < MIN_SUPER_SORTER_FRAMES) {
      throw new TalariaException(new NotEnoughMemoryFault(maxMemoryInJvm, numWorkersInJvm, numProcessingThreadsInJvm));
    }
