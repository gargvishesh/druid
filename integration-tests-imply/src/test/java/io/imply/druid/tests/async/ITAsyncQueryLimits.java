...
public class ITAsyncQueryLimits extends AbstractIndexerTest
{
  ...
  @Test
  public void testClusterAsyncQueryRetentionLimits() throws Exception
  {
    // This test covers the following limits:
    // 1. Retention number of queries limit - total number of queries tracked by Druid. New queries will be rejected.
    List<String> asyncResultIds = new ArrayList<>();
    try {
      final SqlQuery query = new SqlQuery(
          "SELECT 10",
      ...
      AsyncQueryPoolConfig asyncQueryPoolConfig = asyncResourceTestClient.getAsyncQueryPoolConfig();
      for (int i = 0; i < asyncQueryPoolConfig.getMaxAsyncQueries(); i++) {
        SqlAsyncQueryDetailsApiResponse response = asyncResourceTestClient.submitAsyncQuery(query);
        Assert.assertEquals(response.getState(), SqlAsyncQueryDetails.State.INITIALIZED);
        String asyncResultId = response.getAsyncResultId();
        asyncResultIds.add(asyncResultId);
        ITRetryUtil.retryUntilTrue(
            () -> {
              SqlAsyncQueryDetailsApiResponse statusResponse = asyncResourceTestClient.getStatus(asyncResultId);
      ...
      // Now try submit exceeding retention limit. We should get error back since we already have up to retention limit query.
      try {
        asyncResourceTestClient.submitAsyncQuery(query);
        Assert.fail();
      }
