...
export class WorkbenchQuery {
  ...
  public makePreview(): WorkbenchQuery {
    ...
    // Adjust the context, remove msqMaxNumTasks and add in ingest mode flags
    const cleanContext = deleteKeys(this.queryContext, ['msqMaxNumTasks']);
  ...
  public getApiQuery(): {
    ...
    if (engine === 'sql-task') {
      context.msqFinalizeAggregations ??= !ingestQuery;
