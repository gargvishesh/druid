...
function getAggregatorPrefix(fn: SqlFunction): string {
  const functionName = fn.getEffectiveFunctionName();
  switch (functionName) {
    case 'APPROX_COUNT_DISTINCT':
    case 'APPROX_COUNT_DISTINCT_DS_HLL':
    case 'APPROX_COUNT_DISTINCT_DS_THETA':
      return 'unique';

    case 'COUNT':
      return fn.getEffectiveDecorator() === 'DISTINCT' ? 'unique' : 'count';
...
export function convertToAggregateExpression(
  ex: SqlExpression,
  aggregateFunction: SqlFunction,
): SqlExpression {
  return aggregateFunction
    .fillPlaceholders([ex])
    .as(getAggregatorPrefix(aggregateFunction) + '_' + (ex.getOutputName() || 'ex'));
}
